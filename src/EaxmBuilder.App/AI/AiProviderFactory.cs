...
public sealed class AiProviderFactory(SettingsStore settingsStore)
{
    ...
    {
        var profile = settings.ProviderProfiles.TryGetValue(settings.Provider, out var savedProfile)
            ? savedProfile
        ...
        var baseUrl = string.IsNullOrWhiteSpace(profile.BaseUrl) ? settings.BaseUrl : profile.BaseUrl;
        var model = string.IsNullOrWhiteSpace(profile.Model) ? settings.Model : profile.Model;
